...
const Assessment: React.FC = () => {
  ...
  const { ask } = useRagChat(subject || '', chapter || '');
  ...
  const processAnswer = async (answer: string) => {
    ...
    if (currentQ.type === 'multiple-choice') {
      ...
      // Open-ended questions - semantic grading via AI
      const gradingPrompt = `The student answered: '${answer}'. The correct answer is based on: '${currentQ.explanation}'. Did the student demonstrate understanding? Respond with only 'correct' or 'incorrect' followed by one sentence of feedback.`;

      const response = await ask(gradingPrompt);
      isCorrect = response.toLowerCase().startsWith('correct');
      updatedQuestions[currentQuestion].userAnswer = answer;

      // Override feedback for open-ended questions
      feedbackOverride = response;
    }
    ...
    // Provide feedback
    const feedback = feedbackOverride || (isCorrect
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {!isAssessmentStarted ? (
          ...
          <div className="card">
            <div className="mb-6">
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={handleVoiceToggle}
                  className={`flex items-center gap-2 px-6 py-3 rounded-lg transition-colors ${isListening
                      ? 'bg-red-500 text-white'
                      : 'bg-primary-500 text-white hover:bg-primary-600'
                    }`}
                >
          ...
          <div className="text-center">
            <div className="card max-w-2xl mx-auto">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${score >= 70 ? 'bg-secondary-100' : 'bg-yellow-100'
                }`}>
                {score >= 70 ? (
